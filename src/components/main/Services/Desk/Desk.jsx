...
export const Desk = () => {
  ...
  const services = [
    ...
    },
    {
      ...
      serviceText:
        "Написание грамотных коммерческих текстов с целью создания привлекательного информационного содержания на сайте. Продуманный текст поможет привлечь внимание покупателя, ответить на\xA0первичные вопросы и\xA0сподвигнуть совершить целевое действие.",
      imgUrl: COPY,
    ...
    },
    {
      serviceName: "SEO",
      serviceText:
        "СЕО-оптимизация позволяет сайтам быть понятными для\xA0поисковых систем. Качественное СЕО делает ваш сайт выше и\xA0предпочтительнее в\xA0поисковой выдаче. В\xA0таком случае при запросе, пользователь с\xA0большей долей вероятности попадёт в\xA0первую очередь именно к вам.",
      imgUrl: SEO,
      posterUrl: SEOPoster,
    },
    {
      serviceName: "Логотип",
      serviceText:
        "Для того чтобы ваш проект был запоминающимся, ему необходим логотип. Он помогает пользователю быстрее ориентироваться и\xA0запоминать ваш сайт. Также логотип даёт общее представление бренда, кампании и проекта в целом. Пользователю будет гораздо легче узнать вас в будущем.",
      imgUrl: LOGO,
    ...
    },
    {
      serviceName: "Вёрстка",
      serviceText:
        "После создания визуального макета проект переходит в\xA0стадию разработки. Команда разработчиков на основании макета выполняет вёрстку сайта, используя специальные языки, такие как HTML и CSS. В\xA0результате получается быстрый и\xA0адаптивный сайт, который может быть открыт на\xA0любом устройстве.",
      imgUrl: HTMLcoding,
    ...
    },
    {
      ...
      serviceText:
        "При реализации сложного функционала не\xA0обойтись без использования специальных инструментов. Наши разработчики используют для этого самые современные технологии. С\xA0помощью языков программирования прописывается логика работы сайта. Все элементы становятся интерактивными.",
      imgUrl: CODING,
    ...
    },
    {
      ...
      serviceText:
        "Все уведомления, связанные с\xA0сайтом: заявки, данные для\xA0обратной связи или покупка товара, будут приходить на ваш телеграм круглосуточно, с\xA0помощью бота. Вы всегда будете оперативно реагировать на клиента или покупателя. Телеграм-бот идёт в\xA0комплекте к любому проекту.",
      imgUrl: TGBOT,
    ...
    },
    {
      ...
      serviceText:
        "После завершения проекта мы осуществляем гибкую и точную настройку рекламной кампании в\xA0Яндекс Директ. Вы сможете получить первых посетителей в\xA0ближайшие сутки. Полученные данные и\xA0статистика позволят провести анализ для\xA0дальнейшего успешного развития своего проекта.",
      imgUrl: MARKETING,
